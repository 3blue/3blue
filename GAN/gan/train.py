...
def train(D, G, D_solver, G_solver, discriminator_loss, generator_loss, show_every=250, 
    ...
    for epoch in range(num_epochs):
        ...
        for x, _ in train_loader:
            ...
            real_images = next(iter(train_loader))[0]   #No preprocessing seems to yield better images. Why?
            #real_images = x
            ...
            #Discriminator step
            D_solver.zero_grad()
            noise_vector = sample_noise(batch_size, noise_size)
            fake_images = G(noise_vector).view(batch_size, 1, 28, 28)
            #fake_images = preprocess_img(fake_images).to(device) #?
            ...
            if (iter_count % show_every == 0):
                ...
                show_images(imgs_numpy[0:64], color=input_channels!=1)
                plt.show()
                print()
            iter_count += 1